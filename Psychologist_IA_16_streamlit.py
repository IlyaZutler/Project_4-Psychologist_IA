        ...
        while st.session_state.recording:
                ...
                try:
                    ...
                    if text:                                               
                        ...
                        emotion = analyze_emotion(audio_data, source.SAMPLE_RATE) #22050 
...
def main():
    ...
    with DatabaseConnection() as (conn, cursor):
        ...
        if st.button("Stop Recording"):
            st.session_state.recording = False
            st.session_state.was_record = True
        
        emotion_local = ""
        if st.session_state.recording:
            st.write("Position 4:")
            patient_query, emotion_local = recognize_speech(language_full)
            st.write("Position 4b:")
        emotion = emotion_local if emotion_local != "No emotion detected" else emotion
        
        st.write(f"patient_query 5: {patient_query}")
        st.write("Position 6")

        if st.session_state.was_record and not st.session_state.recording:
            st.write(f"Position 7:")
            session_record = update_session_record_query(patient_query, session_record)            
           
            st.write(f"Session record 8: {session_record}")
            st.write(session_record)
            similar_talk, dissimilar_talk = find_similar_talks(llm, model, patient_id, session_record, cursor)            
            response_text = generate_response_llm(llm, session_record, previous_talk, similar_talk, dissimilar_talk, patient_info, emotion)            
